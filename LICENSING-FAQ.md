# Vargate Ogma — Licensing FAQ

This document answers the questions we expect from people evaluating Vargate
Ogma's licensing posture. The authoritative document is
[`LICENSE`](LICENSE); this FAQ is a plain-English companion.

If you want the short version: **you can run Vargate Ogma in production
...
## Can I use Vargate Ogma in production?
...
firm running Vargate Ogma on behalf of a single client under their own
...
We chose BSL because Ogma's value depends on us being a trusted operator
...
## Why does Tyr use Apache 2.0 while Ogma uses BSL?

Vargate **Tyr** is widely-deployable infrastructure for governing autonomous
...
Vargate **Ogma** is a different product. It holds customer prompt and
...
Two products, two postures, two licenses, one company. Tyr is infrastructure;
Ogma is product.
...
managed service whose primary function is the same as Vargate Ogma's
commercial product — i.e., ingesting, analyzing, and reporting on AI usage
data on behalf of paying third-party customers.

It does **not** mean:
- Running Vargate Ogma inside a larger product where AI-usage telemetry
  is one of many features.
- Embedding components of Vargate Ogma inside an internal compliance
  pipeline.
- Offering managed services *that include Vargate Ogma* (e.g., a
  consultancy that deploys and operates Vargate Ogma on a client's own
...
For each version of Vargate Ogma, the BSL terms expire on the **Change
...
## Can I contribute to Vargate Ogma?
...
No. The LICENSE in this repository covers **Vargate Ogma** only. Vargate
Tyr lives in a separate repository under Apache 2.0. Each Vargate product
